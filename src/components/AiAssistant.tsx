    ...
    <>
      {showBubble && !isOpen && (
        <div className="fixed bottom-24 right-6 z-50 animate-fade-up max-w-[260px]">
          <div className="bg-card rounded-2xl rounded-br-sm border border-border p-4 shadow-md cursor-pointer" onClick={() => { setIsOpen(true); setShowBubble(false); }}>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-primary flex items-center justify-center shadow-lg transition-transform duration-200 hover:scale-105"
      >
        {isOpen ? <X className="w-6 h-6 text-primary-foreground" /> : <MessageCircle className="w-6 h-6 text-primary-foreground" />}
      </button>

      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-w-[calc(100vw-3rem)] bg-card rounded-2xl border border-border shadow-lg flex flex-col overflow-hidden animate-scale-in" style={{ height: "min(500px, calc(100vh - 140px))" }}>
          <div className="bg-primary p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-primary-foreground/70">Vastaan kysymyksiisi</p>
            </div>
          </div>
              ...
              <div className="text-center py-6">
                ...
                <div className="space-y-2">
                  {quickQuestions.map(q => (
                    <button key={q} onClick={() => send(q)} className="w-full text-left px-4 py-2.5 rounded-lg border border-border text-sm text-foreground hover:bg-muted/50 transition-colors">
                      {q}
                    </button>
                  ))}
                </div>
          ...
          <div className="p-3 border-t border-border">
            <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="flex items-center gap-2">
              ...
              />
              <button type="submit" disabled={!input.trim() || isLoading} className="w-11 h-11 rounded-lg bg-primary flex items-center justify-center text-primary-foreground disabled:opacity-50 transition-colors hover:bg-primary/90">
